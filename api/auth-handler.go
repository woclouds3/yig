...
func IsBucketPolicyAllowed(userId string, bucket *meta.Bucket, r *http.Request, action policy.Action, objectName string) (allow bool, err error) {
	...
	helper.Logger.Info(r.Context(), "bucket.OwnerId:", bucket.OwnerId, "not equals c.UserId:", userId)
	...
	if policyResult == policy.PolicyAllow {
		helper.Logger.Info(r.Context(),
			"Allow", userId, "access", bucket.Name, "with", action, objectName)
		return true, nil
	} else if policyResult == policy.PolicyDeny {
		helper.Logger.Info(r.Context(),
			"ErrAccessDenied: NotAllow", userId, "access", bucket.Name, "with", action, objectName)
		return false, ErrAccessDenied
	} else {
		return false, nil
	}
