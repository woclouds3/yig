...
type Config struct {
	...
	DbConnMaxLifetime int      `toml:"db_conn_max_lifetime"`
...
func MarshalTOMLConfig() error {
	...
	CONFIG.DbMaxOpenConns = Ternary(c.DbMaxOpenConns == 0, 1024, c.DbMaxOpenConns).(int)
	CONFIG.DbMaxIdleConns = Ternary(c.DbMaxIdleConns == 0, 128, c.DbMaxIdleConns).(int)
	CONFIG.DbConnMaxLifetime = Ternary(c.DbConnMaxLifetime == 0, 300, c.DbConnMaxLifetime).(int)
