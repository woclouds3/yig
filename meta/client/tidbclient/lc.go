...
func (t *TidbClient) PutBucketToLifeCycle(ctx context.Context, lifeCycle LifeCycle) error {
	...
	if err != nil {
		helper.Logger.Println(0, "[", helper.RequestIdFromContext(ctx), "]", "Failed in PutBucketToLifeCycle:", sqltext)
		return nil
	}
	return nil
}
...
func (t *TidbClient) PutBucketToTransition(ctx context.Context, lifeCycle LifeCycle) error {
	...
	if err != nil {
		helper.Logger.Println(5, "[", helper.RequestIdFromContext(ctx), "]", "Failed in PutBucketToTransition:", sqltext, err)
		return nil
	}
	return nil
}
...
func (t *TidbClient) RemoveBucketFromTransition(ctx context.Context, bucket *Bucket) error {
	...
	_, err := t.Client.Exec(sqltext, bucket.Name)
	if err != nil {
		helper.Logger.Printf(0, "[", helper.RequestIdFromContext(ctx), "]", "Failed in RemoveBucketFromTransition: %s\n", sqltext)
		return nil
	}
	return nil
}
...
func (t *TidbClient) ScanTransitionBuckets(ctx context.Context, limit int, marker string) (result ScanLifeCycleResult, err error) {
	...
	if err == sql.ErrNoRows {
		...
		helper.Logger.Println(5, "[", helper.RequestIdFromContext(ctx), "]", err, sqltext, "marker:", marker, time.Now().Add(-1 * DEFAULT_TRANSITION_INTERVAL_HOURS * time.Hour).Format(TIME_LAYOUT_TIDB))
		return
	}
	defer rows.Close()
	result.Lcs = make([]LifeCycle, 0, limit)
	...
	for rows.Next() {
		err = rows.Scan(
			...
			&lc.Status)
		if err != nil {
			helper.Logger.Println(5, "[", helper.RequestIdFromContext(ctx), "]", "Failed in ScanTransitionBuckets: ", rows, result.NextMarker)
			continue
		}
		...
		if _, err = t.Client.Exec(sqltext, time.Now().Format(TIME_LAYOUT_TIDB), lc.BucketName); err != nil {
			helper.Logger.Println(5, "[", helper.RequestIdFromContext(ctx), "]", "Failed in ScanTransitionBuckets: ", lc.BucketName)
			continue
		}
