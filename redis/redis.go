...
func Close() {
	if redisClient != nil && redisClient.IsValid() {
		...
		if err != nil {
			helper.Logger.Error(nil, "Cannot close redis pool, err: %v", err)
		}
	}
}
...
func Remove(table RedisDatabase, prefix, key string) (err error) {
	c, err := GetClient()
	if err != nil {
		helper.Logger.Error(nil, "failed to get redis client, err: %v", err)
		return err
	}
	...
	if err != nil {
		helper.Logger.Error(nil, "failed to call redis del for (%s), err: %v", table.String()+prefix+helper.EscapeColon(key), err)
		return err
	}
	helper.Logger.Info(nil, "Cmd: DEL. Key:", table.String()+key)
	return nil
}
...
func Set(table RedisDatabase, prefix, key string, value interface{}) (err error) {
	c, err := GetClient()
	if err != nil {
		helper.Logger.Error(nil, "failed to get redis client, err:", err)
		return err
	}
	encodedValue, err := helper.MsgPackMarshal(value)
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to make pack for(%s, %s, %v), err: %v", prefix, key, value, err))
		return err
	}
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis Set(%s, %v), err: %v", table.String()+prefix+helper.EscapeColon(key), string(encodedValue), err))
		return err
	}
	helper.Logger.Info(nil, fmt.Sprintf("Cmd: %s. Key: %s. Value: %s. Reply: %s.", "SET", table.String()+key, string(encodedValue), r))
	return nil
}
...
func Get(table RedisDatabase, prefix, key string) (value interface{}, err error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis Get(%s), err: %v", table.String()+prefix+helper.EscapeColon(key), err))
		return nil, err
	}
...
func Keys(table RedisDatabase, pattern string) ([]string, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis Keys(%s), err: %s", query, err))
		return nil, err
	}
...
func MGet(table RedisDatabase, prefix string, keys []string) ([]interface{}, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis MGet(%v), err: %v", queryKeys, err))
		return nil, err
	}
	return results, nil
}
...
func MSet(table RedisDatabase, prefix string, pairs map[string]interface{}) (string, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis MSet(%s, %v), err: %v", prefix, pairs, err))
		return "", err
	}
...
func IncrBy(table RedisDatabase, prefix, key string, value int64) (int64, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis IncrBy(%s, %d), err: %v", prefix+helper.EscapeColon(key), value, err))
		return 0, err
	}
	return result, nil
}
...
func GetBytes(key string, start int64, end int64) ([]byte, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis GetRange(%s, %d, %d), err: %v", FileTable.String()+helper.EscapeColon(key), start, end, err))
		return nil, err
	}
	return value, nil
}
...
func SetBytes(key string, value []byte) (err error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis set(%s), err: %d", FileTable.String()+helper.EscapeColon(key), err))
		return err
	}
	return nil
}
...
func HSet(table RedisDatabase, prefix, key, field string, value interface{}) (bool, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HSet(%s, %s), err: %v", table.String()+prefix+helper.EscapeColon(key), field, err))
		return false, err
	}
	return r, nil
}
...
func HGet(table RedisDatabase, prefix, key, field string) (string, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HGet(%s, %s), err: %v", table.String()+prefix+helper.EscapeColon(key), field))
		return "", err
	}
	return r, nil
}
...
func HDel(table RedisDatabase, prefix, key string, fields []string) (int64, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HDel(%s, %v), err: %v", table.String()+prefix+helper.EscapeColon(key), fields, err))
		return 0, err
	}
	return r, nil
}
...
func HGetInt64(table RedisDatabase, prefix, key, field string) (int64, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HGetInt64(%s, %s), err: %v", theKey, field, err))
		return 0, err
	}
	return r, nil
}
...
func HGetAll(table RedisDatabase, prefix, key string) (map[string]string, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HGetAll(%s), err: %v", table.String()+prefix+helper.EscapeColon(key), err))
		return nil, err
	}
	return r, nil
}
...
func HIncrBy(table RedisDatabase, prefix, key, field string, incr int64) (int64, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HIncrBy(%s, %s, %d), err: %v", table.String()+prefix+helper.EscapeColon(key), field, incr))
		return 0, err
	}
	return r, nil
}
...
func HMSet(table RedisDatabase, prefix, key string, fields map[string]interface{}) (string, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HMSet(%s, %v), err: %v", table.String()+prefix+helper.EscapeColon(key), fields, err))
		return "", err
	}
	return r, nil
}
...
func HMGet(table RedisDatabase, prefix, key string, fields []string) (map[string]interface{}, error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis HMGet(%s, %v), err: %v", table.String()+prefix+helper.EscapeColon(key), fields, err))
		return nil, err
	}
	return r, nil
}
...
func Invalid(table RedisDatabase, key string) (err error) {
	...
	if err != nil {
		helper.Logger.Error(nil, fmt.Sprintf("failed to call redis Public(%s), err: %v", helper.EscapeColon(key), err))
		return err
	}
	return nil
}
